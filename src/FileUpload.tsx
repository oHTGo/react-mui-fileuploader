...
function FileUpload(props: FileUploadProps) {
      ...
      try {
        ...
        for (let i = 0; i < filesTab?.length; i++) {
          ...
          if (allowedExtensions && allowedExtensions.length > 0) {
            const isAllowed =
              allowedExtensions.findIndex((ext: string) => ext.toLowerCase() === extension.toLowerCase()) !== -1
  ...
  const background: string = animate ? theme.palette.secondary.light : theme.palette.primary.light

  return (
    <>
      <Paper sx={{ p: 1 }} elevation={0} ref={filesCardRef} variant="outlined" {...containerCompatibilityProps}>
        <Typography gutterBottom component="div" color="textSecondary" sx={{ display: 'flex' }}>
          ...
          {files?.length > 0 && (
            <Box sx={{ fontSize: 12 }}>
              {files.length}
              {maxUploadFiles > 0 && `/${maxUploadFiles}`} file{files?.length > 1 && 's'} joined
            </Box>
          )}
        </Typography>

        <Paper elevation={0} sx={{ p: 1, transition: 500, background }} {...bannerCompatibilityProps}>
        ...
        {error && (
          <Alert color="error" severity="error" sx={{ mt: 1 }} onClose={() => setError(null)}>
            {error}
          </Alert>
        )}
        ...
        {files?.length > 0 && (
          <React.Fragment>
            ...
            <Typography component="div" align="right" sx={{ mt: 1 }}>
              <Button size="small" onClick={removeFile} ref={buttonDeleteRef}>
